...
func Data(clusterInfo *cluster.Info, status reporter.Interface, options Options, writer io.Writer) bool {
	...
	if _, err := os.Stat(options.Directory); os.IsNotExist(err) {
		...
		if err != nil {
			status.Failure("Error creating directory %q: %s", options.Directory, err)
			return false
		}
	}

	gatherDataByCluster(clusterInfo, status, options, writer)

	_, _ = fmt.Fprintf(writer, "Files are stored under directory %q\n", options.Directory)

	return true
}

func gatherDataByCluster(clusterInfo *cluster.Info, status reporter.Interface, options Options, writer io.Writer) {
	var err error
	clusterName := clusterInfo.Name

	_, _ = fmt.Fprintf(writer, "Gathering information from cluster %q\n", clusterName)
	...
	gatherClusterSummary(&info, writer)
}
