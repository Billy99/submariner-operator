...
func Deployments(cluster *cluster.Info, status reporter.Interface) bool {
...
func checkPods(cluster *cluster.Info, status reporter.Interface) bool {
	...
	submGWErr := checkDaemonset(cluster.ClientProducer.ForKubernetes(), constants.OperatorNamespace, "submariner-gateway", status)
	submRAErr := checkDaemonset(cluster.ClientProducer.ForKubernetes(), constants.OperatorNamespace, "submariner-routeagent", status)
	...
	if cluster.Submariner.Spec.ServiceDiscoveryEnabled {
		lhAgentErr := checkDeployment(cluster.ClientProducer.ForKubernetes(), constants.OperatorNamespace, "submariner-lighthouse-agent", status)
		lhCorednsErr := checkDeployment(cluster.ClientProducer.ForKubernetes(), constants.OperatorNamespace, "submariner-lighthouse-coredns", status)
	...
	if cluster.Submariner.Spec.GlobalCIDR != "" {
		submGNErr := checkDaemonset(cluster.ClientProducer.ForKubernetes(), constants.OperatorNamespace, "submariner-globalnet", status)
