...
func All(cluster *cluster.Info, status reporter.Interface) bool {
	success := K8sVersion(cluster, status)
	...
	success = CNIConfig(cluster, status) && success

	fmt.Println()

	success = Connections(cluster, status) && success

	fmt.Println()

	success = Deployments(cluster, status) && success

	fmt.Println()

	success = KubeProxyMode(cluster, status) && success

	fmt.Println()

	success = FirewallMetricsConfig(cluster, status) && success

	fmt.Println()

	success = VxLANConfig(cluster, status) && success
