...
import (
	...
	"github.com/submariner-io/submariner-operator/pkg/show"
)
...
var (
	...
	connectionsCmd = &cobra.Command{
		...
		Run: func(command *cobra.Command, args []string) {
			execute.OnMultiCluster(restConfigProducer, func(cluster *cluster.Info, status reporter.Interface) bool {
				return show.Connections(cluster, status, os.Stdout)
			})
		},
	}
	endpointsCmd = &cobra.Command{
		...
		Run: func(command *cobra.Command, args []string) {
			execute.OnMultiCluster(restConfigProducer, func(cluster *cluster.Info, status reporter.Interface) bool {
				return show.Endpoints(cluster, status, os.Stdout)
			})
		},
	}
	gatewaysCmd = &cobra.Command{
		...
		Run: func(command *cobra.Command, args []string) {
			execute.OnMultiCluster(restConfigProducer, func(cluster *cluster.Info, status reporter.Interface) bool {
				return show.Gateways(cluster, status, os.Stdout)
			})
		},
	}
	networkCmd = &cobra.Command{
		...
		Run: func(command *cobra.Command, args []string) {
			execute.OnMultiCluster(restConfigProducer, func(cluster *cluster.Info, status reporter.Interface) bool {
				return show.Network(cluster, status, os.Stdout)
			})
		},
	}
	versionsCmd = &cobra.Command{
		...
		Run: func(command *cobra.Command, args []string) {
			execute.OnMultiCluster(restConfigProducer, func(cluster *cluster.Info, status reporter.Interface) bool {
				return show.Versions(cluster, status, os.Stdout)
			})
		},
	}
	allCmd = &cobra.Command{
		...
		Run: func(command *cobra.Command, args []string) {
			execute.OnMultiCluster(restConfigProducer, func(cluster *cluster.Info, status reporter.Interface) bool {
				return show.All(cluster, status, os.Stdout)
			})
		},
	}
)
